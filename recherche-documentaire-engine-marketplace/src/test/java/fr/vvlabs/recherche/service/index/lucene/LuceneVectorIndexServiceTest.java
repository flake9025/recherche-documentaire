...
class LuceneVectorIndexServiceTest {
    ...
    void setUp() {
        ...
        service = new LuceneVectorIndexService(
                ...
                // Chunking desactive: un seul chunk (le contenu complet), sans dependance au tokenizer.
    ...
    @Test
    void addDocumentToDocumentIndex_withChunking_writesOneLuceneDocPerChunkSharingId() throws Exception {
        // Chunker actif avec un tokenizer simule (un token = un mot), fenetre 3, overlap 1.
        // "a b c d e" (5 tokens) -> fenetres [0..3) et [2..5) => 2 chunks.
        String content = "a b c d e";
        ai.djl.huggingface.tokenizers.jni.CharSpan[] spans = {
                new ai.djl.huggingface.tokenizers.jni.CharSpan(0, 1),
                new ai.djl.huggingface.tokenizers.jni.CharSpan(2, 3),
                new ai.djl.huggingface.tokenizers.jni.CharSpan(4, 5),
                new ai.djl.huggingface.tokenizers.jni.CharSpan(6, 7),
        ...
        LuceneVectorIndexService chunkingService = new LuceneVectorIndexService(
                ...
                luceneAutocompleteService,
                bertEmbeddingsService,
                new fr.vvlabs.recherche.service.index.embeddings.chunk.TextChunker(bertEmbeddingsService, true, 3, 1)
        );
        ...
        chunkingService.addDocumentToDocumentIndex(dto, content);

        try (DirectoryReader reader = DirectoryReader.open(directory)) {
            // Deux chunks => deux documents Lucene, tous deux avec l'ID du document.
            assertThat(reader.numDocs()).isEqualTo(2);
            assertThat(reader.storedFields().document(0).get(INDEX_KEY_ID)).isEqualTo("42");
            assertThat(reader.storedFields().document(1).get(INDEX_KEY_ID)).isEqualTo("42");

            FloatVectorValues vectors = reader.leaves().getFirst().reader().getFloatVectorValues(VECTOR_FIELD);
            assertThat(vectors.size()).isEqualTo(2);
        }
    }
}
