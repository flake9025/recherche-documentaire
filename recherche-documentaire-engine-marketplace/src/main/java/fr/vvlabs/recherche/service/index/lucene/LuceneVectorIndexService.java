...
import static fr.vvlabs.recherche.config.IndexConstants.INDEX_KEY_CHUNK_INDEX;
...
public class LuceneVectorIndexService implements IndexService<ByteBuffersDirectory> {
    ...
    private final fr.vvlabs.recherche.service.index.embeddings.chunk.TextChunker textChunker;
        ...
        synchronized (luceneConfig.getDocumentsIndexLock()) {
            try (IndexWriter writer = new IndexWriter(luceneConfig.getDocumentsIndex(), config)) {
                ...
                for (fr.vvlabs.recherche.service.index.embeddings.chunk.TextChunk chunk : chunks) {
                    ...
                    doc.add(new StoredField(INDEX_KEY_CHUNK_INDEX, chunk.index()));
                    ...
                    doc.add(new TextField(INDEX_KEY_CONTENT, chunk.text(), Field.Store.YES));
                    // Un vecteur vide (contenu vide) n'est pas indexable en KNN: on n'ajoute alors pas le champ vecteur.
                    if (vector.length > 0) {
