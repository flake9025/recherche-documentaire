...
- `milvus` existe encore en placeholder et jette `UnsupportedOperationException`.
...
- Profils Docker/Linux: `application-lucene.yml`, `application-bert.yml`, `application-lucene-vector.yml` (forcent `tesseract.dataPath=/usr/share/tessdata`).
- Le bean S3 est conditionnel `app.storage.s3.enabled=true` (`service/storage/s3/AmazonS3StorageService.java`), sinon `FSStorageService`.
...
- CI GitHub: `/.github/workflows/build.yml` valide `lucene`, `faiss` et `qdrant` par smoke tests Docker/Compose avant publication
